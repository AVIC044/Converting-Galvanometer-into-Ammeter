...
public class RheostatSlideController : MonoBehaviour
{
    ...
    [Header("Digital Display Parents")]
    [Tooltip("Display parent for the voltmeter UI/Container")]
    [SerializeField] private GameObject meterDisplayParent1;

    [Tooltip("Display parent for the galvanometer UI/Container")]
    [SerializeField] private GameObject meterDisplayParent2;
    ...
    private void OnPageChanged(int pageIndex)
    {
        ...
        // 2. If it is a target slide, enable meters and setup UI
        if (matchedIndex != -1)
        {
            SetActivePoint(matchedIndex);

            // Force BOTH meter displays ON
            SetAllMeterDisplaysActive(true);
    ...
    private void UpdateMeterDisplayVisibility(int currentPageIndex)
    {
        // Turn ON both displays simultaneously on active slide indices
        SetAllMeterDisplaysActive(true);
    }
